...
final class HiddenApi {
	...
	static {
		oneArray = (Object[]) VMRuntime.getRuntime().newNonMovableArray(Object.class, 1);

		/* Method is a pointer with a size equivalent to int. */
		int[] ints = (int[]) runtime.newNonMovableArray(int.class, 0);
		offset = runtime.addressOf(ints) - vaddressOf(ints);
	}
	...
	public static void setHiddenApiExemptions() {
		...
		try {
			if (setHiddenApiExemptions == null) {
				throw new IllegalStateException("setHiddenApiExemptions not found.");
			}
	...
	private static Method findMethod() {
		...
		for (int k = 0; k < length; ++k) {
			final long kAddress = address + k * intBytes;
			final long addressMethod = unsafe.getInt(kAddress);
			final long addressArtMethod = unsafe.getLong(addressMethod + offsetArtMethod);
		...
		if (sizeArtMethod > 0 && sizeArtMethod < 100) {
			for (long artMethod = minSecond; artMethod < max; artMethod += sizeArtMethod) {
				...
				unsafe.putLong(addressMethod + offsetArtMethod, artMethod);
	...
	static void replaceAMethod(Method dest, Method src) {
		long addressDest = unsafe.getLong(dest, offsetArtMethod);
		long addressSrc = unsafe.getLong(src, offsetArtMethod);
	...
	public static class Delegator extends Super {
		@Override
		public Class<?> findLoadedClass(String name) {
			if (name.equals("java.lang.Exception")) return Exception.class;
